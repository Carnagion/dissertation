...
#show raw.where(block: true): set block(fill: luma(240), inset: 1em, radius: 5pt, width: 100%)
...
Exploiting complete orders by creating ordered sets of aircraft simplifies the problem of runway sequencing (or more generally, machine scheduling), as a solution can simply interleave these ordered sets, always sequencing only the first available aircraft from each set @demaere-pruning-rules.
...
The full OPL source code of the model is available in @appendix:opl.

Typically, time-indexed formulations like the one used in this model leads to a large number of binary decision variables, which in turn produces a large number of combinations of values that must be created, stored, and iterated.
However, not all of these combinations are valid -- for example, it is always invalid to schedule an aircraft outside of its time window.
Using only these valid combinations creates _sparse model data_ which significantly reduces the size of the model, its memory consumption, and the time needed to solve it.
Sparse data is used at multiple places throughout the model, some of which are shown in @code:sparse-data.

#let sparse-data = ```cs
// Restrict the binary variables for landing and take-off times to only the valid combinations
...
// Restrict the binary variables for de-icing times to only the valid combinations
...
// Apply constraints to only valid combinations
...
#figure(
    sparse-data,
    caption: [Use of sparse data in the OPL implementation of the model],
) <code:sparse-data>

Taking an example from @code:sparse-data -- a `FlightTimePair` defines a valid combination of an aircraft and a time that it can be scheduled to land or take-off.
Using this, the binary decision variable $tau_(i, t)$ -- named `isScheduledAt` in the OPL code -- can be restricted to only the valid combinations of aircraft and landing or take-off times for that aircraft, reducing the number of values that need to be processed at any given time.
A similar approach is utilised for the de-icing times and for the constraints, for which sets of valid combinations of aircraft pairs are calculated beforehand.
...
Rust was chosen due to its low memory footprint and high base performance, which stems from the lack of a runtime or garbage collector.
At the same time, its unique ownership-based memory model and high-level type system minimise bugs at compile time, allowing for fast iterative development.
...
#let cplex-heathrow-total-runtime = cplex-heathrow-benches.values().map(bench => bench.stats.mean.point-estimate).sum()
#let cplex-heathrow-avg-runtime = avg(..cplex-heathrow-benches.values().map(bench => bench.stats.mean.point-estimate))

#let branch-bound-heathrow-avg-runtimes-small = heathrow-avg-runtimes(1, 10).pairs().fold(
    (:),
    (dict, (key, total)) => dict + ((key): calc.round(total / 1000, digits: 2)),
)

It can be clearly seen that the mathematical program implemented in CPLEX is nearly four orders of magnitude slower than the branch-and-bound program.
Indeed, the total time taken by CPLEX to solve all small Heathrow problem instances is #calc.round(cplex-heathrow-total-runtime / 1000000000, digits: 2) seconds, which equates to an average runtime of #calc.round(cplex-heathrow-avg-runtime / 1000000000, digits: 2) seconds.
In comparison, the average runtime taken by the branch-and-bound program using the integrated approach to solve the same instances is #branch-bound-heathrow-avg-runtimes-small.integrated microseconds.
...
// NOTE: Appendix hidden since it's not supposed to be submitted with the main body of the text
#hide[
    ...
    = Source Code of Mathematical Program <appendix:opl>
]
