...
The aforementioned datasets were developed solely for the runway sequencing problem and not integrated runway and de-icing sequencing. This meant that the instances did not contain data for the pushback durations, taxi durations, de-icing durations, and line-up durations of aircraft, making them unsuitable for this project. Thus, there was a need to augment the datasets and create a data generator.

=== Data Preprocessing

// TODO: Probably reference the paper by Psaraftis
Before sequencing, an instance is first split into sets of _separation-identical_ aircraft as a preprocessing step. Two aircraft $A_i$ and $A_j$ are separation-identical if their mutual separations with respect to every other aircraft $A_k$ in the set of aircraft $A$ are the same @demaere-pruning-rules; i.e. $A_i$ and $A_j$ are separation-identical if and only if:
...
#pseudocode(
    ...
    [*for* $i$ *in* $A$ *do*], ind,
        [*for* $s$ *in* $S$ *do*], ind,
            ...
            [add $i$ to $s$],
            [*continue* to next aircraft in $A$],
